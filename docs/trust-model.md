...
## A future live attach relay stays content-blind; the sync anchor does not

Super Chat remote attach adds a second, live channel beside CRDT sync, and the two have different confidentiality shapes that must not be conflated (ADR-0115). The distinction is the whole point.
...
- **The live attach relay forwards sealed frames it cannot read.** It carries a Super Chat session (prompts, tool results, approvals) between two of your own signed-in devices, addressed by `principalId`, `hostId`, `deviceId`, and `attachId`. It holds no keys and parses no session commands; it sees the envelope (which devices, when, how many bytes), never the contents. It is endpoint-addressed, not route-addressed, so it is not a resurrection of the deleted relay floor (ADR-0086, ADR-0115).

Confidentiality still follows topology. On Cloud, attach frames are always sealed, with key agreement authenticated to the device grant, so the operator forwards ciphertext it cannot read and cannot man-in-the-middle. On self-host the operator is you, so plaintext is a fail-closed opt-out that only a self-host operator can take. The relay is the same either way; sealing is a layer above it, on by default on Cloud.
