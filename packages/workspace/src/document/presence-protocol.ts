 ...
 * payload is each peer's `nodeId`, `connectedAt`, and `agentId`. The legacy
 * action-manifest field was deleted with the in-room dispatch subsystem
 * (ADR-0073); actions live in the local registry (`collaboration.actions`),
 * never on the wire.
 *
