 ...
 * workspaces should pass a principal-scoped key (`attachLocalStorage` derives
 * one from the `(server, principalId)` pair) so two signed-in principals in the
 * same browser profile cannot exchange plaintext.
 *
