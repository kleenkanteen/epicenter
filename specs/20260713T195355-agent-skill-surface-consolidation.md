...
- [ ] Rename `skill-creator` to `agent-instructions` without rewriting its body.
- [ ] Rename `approachability-audit` to `first-read-review` without rewriting its body.
- [ ] Copy instruction-placement guidance verbatim into its proposed final reference while leaving the old entrypoint intact.
