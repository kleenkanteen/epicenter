...
| Auth model | 1 evidence | Two tokens: a single-use bootstrap token in the URL fragment, exchanged at `POST /api/session` for a per-launch session bearer stored in sessionStorage | Fragments never reach the server (no request or access logs, no Referer); sessionStorage survives F5 within the tab. Mail bodies render only after the SPA sanitizer strips script-capable markup, event handlers, and remote assets. Full model in Security |
...
- **Why sessionStorage and not in-memory**: in-memory dies on F5 or tab restore, and with the bootstrap spent the user would be locked out until `up` restarts (the previous draft never resolved this, and its Edge Cases already implied two tokens without saying so). sessionStorage survives reload within the tab and dies with the tab. The app origin has no mail-body script-injection vector because raw `text/html` is sanitized before the single render sink; scripts, event handlers, remote assets, and other active markup are stripped before render (see rendering below).
...
- **Implementation note (2026-07-07)**: Phase C shipped inline sanitized rendering instead of an iframe `srcdoc` frame. The invariant is now one component boundary: `MessageBody.svelte` is the only mail-body render sink, and the only string permitted to reach its `{@html}` site is DOMPurify output from `sanitizeEmailHtml`.
- **Sanitizer, non-negotiable**: DOMPurify runs before raw `text/html` ever reaches the render sink. DOMPurify handles the executable surface by default (scripts, event handlers, and `javascript:` URLs). Local Mail additionally forbids `style`, `link`, and `base` tags, strips fetch-capable attributes such as `src`, `srcset`, `background`, `poster`, and `lowsrc`, and removes inline styles that contain `url(...)`. Links are forced to open in a new tab with `rel="noopener noreferrer"`. Raw mail HTML is never rendered directly.
- **Containment**: formatted email renders on a bounded light canvas inside the app. Wide media and tables are capped to the canvas width so hostile layout cannot force the app to scroll horizontally.
- **Show-images precision**: enabling images for a message re-enables CSS `url()` and therefore CSS attribute-selector exfiltration for that message, not just tracking pixels. Rule: "show images" allows `img` elements only; CSS `url()` in style attributes and blocks stays stripped even in show-images mode. Recommended (decide in a later phase): route shown images through the local Bun process as a stripping proxy, so `img-src` stays loopback-only and remote hosts never see the client IP.
...
- [ ] **C.2** Body rendering behind the sanitizer + single-`{@html}` invariant (see Security). Decide the show-images proxy here.
