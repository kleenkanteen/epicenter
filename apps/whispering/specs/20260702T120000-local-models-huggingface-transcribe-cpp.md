...
Phase 1 step 1 called the cross-platform Cargo/CI matrix "the real cost of the whole effort." A spike pass proved it out. Summary of the finding and the packaging wiring that was added.

### Build status per target

| Target | transcribe-cpp posture | Link | Files to ship beside the binary | Build proof |
| --- | --- | --- | --- | --- |
| macOS aarch64 | `metal` | **static** (`GGML_METAL_EMBED_LIBRARY=ON`) | none | ✅ `cargo check --lib` green; native build cached |
| x86_64 Linux gnu | `dynamic-backends` + `vulkan` | **shared** + backend-DL | `libtranscribe.so` + ggml backend modules (Vulkan + per-ISA CPU) | ⏳ compiles on CI (Vulkan SDK + CMake already installed by `setup-whispering-build`); runtime unproven — see smoke below |
| x86_64 Windows msvc | `dynamic-backends` + `vulkan` | **shared** + backend-DL | `transcribe.dll` + ggml DLLs + per-ISA CPU modules | ⏳ same as Linux (Vulkan SDK + LLVM already installed) |
...
**Local cross-build is not feasible** (host is `aarch64-apple-darwin`, no cross targets / `cross` / `zig`; the sys crate is a native CMake ggml build linking the Vulkan SDK, and MSVC cannot be produced from macOS). Each non-mac target must build on its own native runner — which the release matrix already provides.
...
The sys crate forwards its output dirs as `DEP_TRANSCRIBE_CPP_RUNTIME_DIR` / `_MODULE_DIR` (present only in the shared posture). Wired end-to-end:

- **`src-tauri/build.rs`** — copies the runtime libs + backend modules into a stable, git-ignored `src-tauri/transcribe-libs/` (by name, dereferencing Linux SONAME symlinks), and **hard-fails** if a shared build advertised a dir but staged zero files. Inert on the static targets (env vars unset → folder stays empty). On Unix it also emits an rpath so `libtranscribe` is found: the sys lib dir for `tauri dev`, and `$ORIGIN/../lib/Whispering` for the Linux bundle layout.
- **`tauri.conf.json` `resources`** — switched to the **map form** so `"transcribe-libs/*": ""` lands the files in the resource **root**, which is the executable's own directory on Windows (satisfying the load-time DLL search). A committed `transcribe-libs/README.md` keeps the glob from matching zero files (tauri-build 2.6.2 hard-errors on a zero-match resource glob). The recorder-state PNGs were converted to explicit map entries with identical runtime paths.
- **`gguf_spike.rs`** — `init_transcribe_cpp_backends` calls `init_backends_default()`, which scans the directory of the loaded `libtranscribe` for its modules. That is layout-correct on every target because the bundle stages `libtranscribe` and its modules into the same resource root (`transcribe-libs/* -> ""` flattens both beside the executable, confirmed against `tauri-utils` 2.9.2 source), and a dev build loads `libtranscribe` from the sys crate's own output dir where the modules were just built. No-op on static builds. (An earlier draft passed an explicit `resource_dir()/transcribe-libs` path; that was wrong: the `""` target flattens to the root, so the subfolder never exists and the branch was dead.)

### Residual gate before deleting transcribe-rs (Phase 1 step 4)

macOS is proven and inert. The non-mac path is compile-ready and packaging-wired, but its **runtime** is unproven from this host. Before the transcribe-rs deletion in step 4 is safe, CI must add a **runtime smoke on x86_64 Linux and x86_64 Windows**: build the bundle, launch it, assert `transcribe_cpp::devices()` ≥ 1 and one GGUF transcription succeeds. That smoke also confirms the two Tauri-layout assumptions this host cannot check: the Windows DLL-beside-exe placement from the `""` resource target, and the Linux `$ORIGIN/../lib/Whispering` rpath resolving to the real resource dir (`<mainBinaryName>` must match). Until that is green on both, deleting transcribe-rs would ship a zero-device regression on Linux and Windows. macOS may proceed independently.
