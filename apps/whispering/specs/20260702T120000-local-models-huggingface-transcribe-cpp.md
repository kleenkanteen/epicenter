...
macOS is proven and inert, and the non-mac bundles now build + audit green (Linux `/usr/lib` flat; Windows `transcribe.dll` + `ggml-cpu-*.dll` beside the exe in staging / MSI / NSIS). The Windows compile blocker is resolved (SPIRV-Headers via vcpkg). Before the transcribe-rs deletion in step 4 is safe, two gates remain:

1. **Windows VC++ runtime staging (open question, *not* `vcomp140`).** The native DLLs are built `-MD` (dynamic C++ runtime), so `msvcp140.dll` / `vcruntime140.dll` must exist on the user's machine; Whispering stages neither and relies on the VC++ redistributable being present. Note the build sets `TRANSCRIBE_USE_OPENMP=OFF`, so the ggml-cpu modules do **not** link OpenMP and `vcomp140.dll` is **not** needed (unlike Handy, which builds with OpenMP and stages it). CI runners have the redist, so neither the build nor the artifact audit can catch a missing-redist regression on an end-user machine — decide whether to stage `msvcp140`/`vcruntime140` beside the exe.
2. **Runtime `devices()` smoke on x86_64 Linux and x86_64 Windows.** Build the bundle, launch it, assert `transcribe_cpp::devices()` ≥ 1 and one GGUF transcription succeeds. Whispering has no `--list-devices` CLI (Handy's smoke), so this needs a Whispering-native harness. The static artifact audit is a weaker proxy: it proves the libs are *in* the package beside the loader, not that the loader resolves them and registers a device — and it is also where the open `Backend::Vulkan`-vs-CPU-fallback behaviour on a GPU-less Windows box would finally be exercised.

The audit confirms placement statically on both platforms; the `devices()` smoke is what proves the loader actually finds and registers a compute backend. Until both gates are green, deleting transcribe-rs would risk a zero-device Windows regression on machines without the VC++ redist or a usable GPU path. macOS may proceed independently.
