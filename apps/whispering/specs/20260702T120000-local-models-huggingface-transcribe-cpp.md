...
Phase 1 step 1 called the cross-platform Cargo/CI matrix "the real cost of the whole effort." A spike pass proved the macOS API path, and a **runner-proof preview CI pass** (PR #2304) then exercised the non-mac bundles for the first time. It found one hard blocker the local host could not see (Windows does not compile) and confirmed the Linux bundle layout end to end. The findings and the committed packaging wiring are below.
...
3. **Runtime `devices()` smoke on x86_64 Linux and x86_64 Windows.** Build the bundle, launch it, assert `transcribe_cpp::devices()` ≥ 1 and one GGUF transcription succeeds. Whispering has no `--list-devices` CLI (Handy's smoke), so this needs a Whispering-native harness. The static artifact audit added in this pass is a weaker proxy: it proves the libs are *in* the package at the rpath location, not that the loader resolves them.

On Linux the audit now confirms the libs ship flat in `/usr/lib` matching the `$ORIGIN/../lib` rpath, but that is a static placement check; the `devices()` smoke is what proves the loader actually finds and registers them. Until items 1–3 are green, deleting transcribe-rs would ship a broken or zero-device Linux/Windows build. macOS may proceed independently.
