...
Phase 1 step 1 called the cross-platform Cargo/CI matrix "the real cost of the whole effort." A spike pass proved the macOS API path, and a **runner-proof preview CI pass** (PR #2304) then exercised the non-mac bundles for the first time. As of the latest run **all four rows are green**: it caught (and the diagnostic dump corrected) an audit parse bug on Linux, surfaced a Windows compile blocker (missing SPIRV-Headers, since fixed via vcpkg), and now confirms the Linux and Windows bundle layouts end to end. The findings and the committed packaging wiring are below.
...
| x86_64 Linux gnu | `dynamic-backends` + `vulkan` | **shared** + backend-DL | `libtranscribe.so` + ggml backend modules (Vulkan + per-ISA CPU) | ✅ builds + bundles green. The artifact audit confirmed all three packages (deb / rpm / AppImage) ship `libtranscribe.so` + the per-ISA `libggml-cpu-*.so` backends **flat in `/usr/lib`** (Tauri copies the directory's contents into the target), matching the `$ORIGIN/../lib` rpath. Runtime `devices()` smoke now wired into preview CI (the `transcribe_cpp_devices` example, run against the staged libs). |
| x86_64 Windows msvc | `dynamic-backends` + `vulkan` | **shared** + backend-DL | `transcribe.dll` + ggml DLLs + per-ISA CPU modules | ✅ builds + bundles green after installing **SPIRV-Headers via vcpkg** (ggml-vulkan does `find_package(SPIRV-Headers CONFIG REQUIRED)`; the Vulkan SDK ships the loader but not that config). Audit confirms `transcribe.dll` + `ggml-cpu-*.dll` land beside the exe in the staging dir, the WiX MSI, and the NSIS installer. Runtime `devices()` smoke now wired into preview CI. VC++ runtime staging decision closed (rely on the redist; do not stage — see residual gate). |
...
**Local cross-build is not feasible** (host is `aarch64-apple-darwin`, no cross targets / `cross` / `zig`; the sys crate is a native CMake ggml build linking the Vulkan SDK, and MSVC cannot be produced from macOS). Each non-mac target must build on its own native runner — which the release matrix already provides. This is exactly why the Windows build failure stayed invisible until a real runner built the bundle.
...
- **`src-tauri/build.rs`** — copies the runtime libs + backend modules into a stable, git-ignored `src-tauri/transcribe-libs/` (by name, dereferencing Linux SONAME symlinks), and **hard-fails** if a shared build advertised a dir but staged zero files. Inert on the static targets (env vars unset → folder stays empty). On Linux it also emits the rpath so `libtranscribe` is found: the sys lib dir for `tauri dev`, and **`$ORIGIN/../lib`** for the installed bundle (binary in `/usr/bin`, libs flat in `/usr/lib`). Windows needs no rpath (DLLs resolve from the exe dir) and macOS is static.
- **Linux bundle (`tauri.conf.json`)** — `bundle.linux.{deb,rpm,appimage}.files` maps `{"/usr/lib": "transcribe-libs"}`. Tauri copies the **contents** of the `transcribe-libs` directory into `/usr/lib`, so the libs land flat (`/usr/lib/libtranscribe.so`, `/usr/lib/libggml-cpu-*.so`), confirmed against all three built packages in CI. (An earlier draft of the audit misread a deb-listing parse bug — `dpkg-deb --contents | awk '{print $NF}'` dropped the leading `/usr` on this runner — as a nesting bug; the packages were flat all along. The audit now lists the deb via `dpkg-deb --fsys-tarfile | tar -t`.)
- **Windows bundle (`tauri.windows.conf.json`)** — `bundle.resources` maps `{"transcribe-libs": "."}`. `bundle.resources` flattens a directory-to-`"."` mapping into the `$RESOURCES` root, which on Windows is the executable's own directory (confirmed against `tauri-utils` resource-walk tests), satisfying the load-time DLL search. This merges additively with the base `resources` (the recorder-state PNGs) via the platform-config JSON Merge Patch. The audit confirms this layout: `transcribe.dll` + the `ggml-cpu-*.dll` backends are present beside the exe in the staging dir, the MSI, and the NSIS installer. (Static placement only; the `devices()` smoke is the runtime proof.)
- **Windows build toolchain (`setup-whispering-build`)** — the x64 build additionally installs `spirv-headers` via vcpkg and prepends its prefix to `CMAKE_PREFIX_PATH`, because ggml-vulkan does `find_package(SPIRV-Headers CONFIG REQUIRED)` and the Vulkan SDK does not expose that config. Without it the native build aborts at configure time.
- **`gguf_spike.rs`** — `init_transcribe_cpp_backends` calls `init_backends_default()`, which scans the directory of the loaded `libtranscribe` for its modules. Layout-correct on every target because the bundle stages `libtranscribe` and its modules into the **same** directory: `/usr/lib` on Linux (reached by the rpath), the exe dir on Windows, and the sys crate's own output dir for a dev build. No-op on the static builds.

### Residual gate before deleting transcribe-rs (Phase 1 step 4)

macOS is proven and inert, and the non-mac bundles now build + audit green (Linux `/usr/lib` flat; Windows `transcribe.dll` + `ggml-cpu-*.dll` beside the exe in staging / MSI / NSIS). The Windows compile blocker is resolved (SPIRV-Headers via vcpkg). The two gates that stood before the transcribe-rs deletion in step 4 are now closed:

1. **Windows VC++ runtime staging — DECIDED: rely on the redistributable, do not stage.** Verified against `transcribe-cpp-sys`: the native ggml/transcribe DLLs are built `-MD` (dynamic CRT — no `MSVC_RUNTIME_LIBRARY` override, `GGML_STATIC` OFF, `.profile("Release")` selects `/MD`), so `msvcp140.dll` / `vcruntime140.dll` must exist on the user's machine. Whispering stages neither and relies on the VC++ Redistributable being present, matching Handy for these two DLLs. **`vcomp140.dll` is confirmed *not* needed:** `TRANSCRIBE_USE_OPENMP` defaults `OFF` (`transcribe-cpp-sys/CMakeLists.txt`; the `openmp` feature is not enabled here), so the ggml-cpu modules use ggml's native CPU threadpool and link no OpenMP runtime (unlike Handy, which builds with OpenMP and therefore stages `vcomp140`). We do **not** stage `msvcp140`/`vcruntime140` now: CI runners carry the redist so no build or audit can exercise a redist-absent machine, and staging DLLs no live failure demands is speculative (repo doctrine: minimal shape, add the seam when a producer exists). **Trigger to revisit:** the first end-user report of a missing `msvcp140.dll` / `vcruntime140.dll`. App-local deployment of exactly those two DLLs is Microsoft-supported, so the remedy stays cheap (copy them beside the exe via `build.rs`, no installer merge module).
2. **Runtime `devices()` smoke on x86_64 Linux and x86_64 Windows — WIRED into preview CI.** Whispering has no `--list-devices` CLI (Handy's smoke), so this is a Whispering-native harness: `src-tauri/examples/transcribe_cpp_devices.rs` calls transcribe-cpp's own `init_backends*` and asserts `transcribe_cpp::devices()` ≥ 1, exiting non-zero on failure. `pr-preview.whispering.yml` runs it on the Linux and Windows rows after the static audit, pointing the OS library-search path (and the harness's explicit module-scan arg) at the staged `transcribe-libs` dir, so it proves the loader resolves the **exact files the installer ships** and registers a compute backend. It reuses the release artifacts tauri-action just built (an `example`, not a test — no `panic=unwind` harness rebuild, no per-profile native cmake rebuild). A GPU-less Windows runner exercises the `Backend::Vulkan`-vs-CPU-fallback path via the CPU backend module. Batch-transcription of a GGUF is not part of the smoke (it would pull a model download into CI); device registration is the loader proof this gate needed.

The audit confirms placement statically on both platforms; the `devices()` smoke proves the loader actually finds and registers a compute backend. With the smoke green and the VC++ decision recorded, the step-4 transcribe-rs deletion no longer risks a silent zero-device Linux/Windows regression. macOS remains static, proven, and independent.
