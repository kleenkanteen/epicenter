 ...
 * (`events.shortcutTriggerEvent`, `events.shortcutCaptureEvent`) binds the
 * topic name to its payload type, so call sites listen without hand-mirroring
 * the Rust topic string. Re-exported raw: events carry plain payloads, so there
 * is no `Result` shape to convert.
 */
export { events };
